<template>
  <div class="admin-training-types">
    <div class="header-section">
      <h1 class="page-title">Tipologias de Capacitação</h1>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="type in types" :key="type.id">
              ...
              <td>
                <div class="actions">
                  <button @click="openModal(type)" class="btn-icon edit" title="Editar">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                  </button>
                  <button @click="deleteType(type)" class="btn-icon delete" title="Excluir">
      ...
      <div class="modal">
        ...
        <form @submit.prevent="saveType" class="modal-body">
          ...
          <div class="form-group">
            <label>Nome</label>
            <input v-model="form.name" type="text" required placeholder="Ex: Curso de Curta Duração" />
          </div>
...
const types = ref([]);
const activeTypes = computed(() => types.value.filter(t => t.status === 'active').length);
...
const saveType = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      await axios.put(`/api/admin/training-types/${currentId.value}`, form, { headers });
    } else {
      await axios.post('/api/admin/training-types', form, { headers });
    }
    await loadTypes();
    closeModal();
  } catch (error) {
...
const deleteType = async (type) => {
  if (!confirm(`Tem certeza que deseja excluir "${type.name}"?`)) return;

  try {
    await axios.delete(`/api/admin/training-types/${type.id}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    });
    await loadTypes();
  } catch (error) {
