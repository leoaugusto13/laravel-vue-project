  ...
  <div class="cities-page">
    <div class="page-header">
      <div class="header-content">
        ...
        <p>Gerencie os municípios disponíveis para capacitação.</p>
      </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="city in cities" :key="city.id">
            ...
            <td>{{ city.state ? city.state.uf : 'N/A' }}</td>
            <td>
              <span class="status-badge" :class="city.active ? 'active' : 'inactive'">
                {{ city.active ? 'Ativo' : 'Inativo' }}
              </span>
            </td>
            <td class="actions-cell">
              <button @click="openEditModal(city)" class="icon-btn edit" title="Editar">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
              </button>
              <button @click="deleteCity(city)" class="icon-btn delete" title="Excluir">
        ...
        <div class="modal-body">
          <form @submit.prevent="saveCity">
            <div class="form-group">
              ...
              <input v-model="form.name" type="text" class="form-input" required placeholder="Ex: Rio de Janeiro">
            </div>
            ...
            <div class="modal-footer">
              ...
              <button type="submit" class="btn-save">{{ isEditing ? 'Salvar Alterações' : 'Criar Município' }}</button>
            </div>
          </form>
        </div>
      </div>
...
const activeCities = computed(() => cities.value.filter(c => c.active).length);
...
const fetchStates = async () => {
  try {
    ...
    states.value = response.data.filter(s => s.active); // Only show active states
  } catch (error) {
...
const saveCity = async () => {
  try {
    ...
    if (isEditing.value) {
      await axios.put(`/api/admin/cities/${form.id}`, payload, { headers });
    } else {
      await axios.post('/api/admin/cities', payload, { headers });
    }
    
    await fetchCities();
    closeModal();
    alert(isEditing.value ? 'Município atualizado!' : 'Município criado com sucesso!');
  } catch (error) {
    console.error('Erro ao salvar:', error);
    alert('Erro ao salvar o município. Verifique os dados.');
  }
};

const deleteCity = async (city) => {
  if (!confirm(`Tem certeza que deseja excluir o município "${city.name}"?`)) return;
...
.city-info .name { font-weight: 600; }
